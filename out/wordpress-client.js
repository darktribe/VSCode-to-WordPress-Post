...
class WordPressClient {
        ...
        try {
            ...
            if (metadata.slug) {
                ...
                if (existingPostId) {
                    ...
                    return await this.updateExistingPost(existingPostId, title, content, metadata, featuredImageId);
                }
                else {
     ...
     * 既存記事を更新
     */
    async updateExistingPost(postId, title, content, metadata, featuredImageId) {
        try {
     ...
     * 新規記事作成
     */
    async createNewPost(title, content, metadata, featuredImageId) {
        // カテゴリ名をIDに変換
        let categoryIds = [];
        const categoryNames = this.extractCategoryNames(metadata);
        if (categoryNames.length > 0) {
     ...
     */
    async convertCategoriesToIds(categoryNames) {
        const categoryIds = [];
        // 既存カテゴリを取得
     ...
     * 投稿後の確認
     */
    async verifyPost(postId, expectedCategoryNames, actualCategoryIds) {
